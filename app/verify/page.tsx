...
function VerifyPageContent() {
  ...
  const [searchType, setSearchType] = useState<"cert" | "tx" | "wallet">(
    "cert",
  );
  ...
  const [documentVerifyResult, setDocumentVerifyResult] =
    useState<DocumentVerificationResult | null>(null);
  const [documentVerifyError, setDocumentVerifyError] =
    useState<DocumentVerifyError | null>(null);
  ...
  const doSearch = useCallback(
    async (query: string, type: "cert" | "tx" | "wallet") => {
  ...
  const handleDocumentVerify = async (e: React.FormEvent) => {
    ...
    if (!documentFile) {
      setDocumentVerifyError({
        kind: "input",
        title: "Please select a PDF first",
        message: "Drop or choose a certificate PDF to check.",
        nextStep: "Select a PDF file and try again.",
      });
      return;
    }
    ...
    try {
      const verification = await verifyCertificateDocumentFile(
        certId || null,
        documentFile,
      );
      ...
      if (err instanceof ApiError) {
        ...
        if (err.status === 404) {
          setDocumentVerifyError({
            ...
            nextStep:
              "Ask the issuer for the original certificate, QR code, or certificate ID.",
          ...
          setDocumentVerifyError({
            ...
            nextStep:
              "Contact the issuing institution to confirm this certificate.",
            certId: resolvedCertId,
          });
          ...
          setDocumentVerifyError({
            ...
            message:
              "We found this certificate, but our connection to the stored original file is temporarily unavailable.",
          ...
          setDocumentVerifyError({
            ...
            message: baseMessage,
            nextStep: "Try again. If this keeps happening, contact support.",
            certId: resolvedCertId,
            uploadedSha256,
          });
        }
        ...
        setDocumentVerifyError({
          ...
          title: "Verification failed",
          message:
            err instanceof Error ? err.message : "File verification failed",
          nextStep: "Try again.",
  ...
  const documentVerdict = documentVerifyResult
    ? documentVerifyResult.verified
      ? {
          ...
          title: "Authentic certificate confirmed",
          message:
            "The PDF you uploaded matches the original certificate on file.",
          nextStep: "You can trust and accept this certificate.",
        }
      : documentVerifyResult.match.sha256
        ? {
            tone: "warning" as const,
            title: "File matches, but certificate is no longer active",
            message:
              "The PDF is the original file, but this certificate has been marked as inactive.",
            nextStep: "Contact the issuing institution before accepting it.",
          }
        : {
            tone: "danger" as const,
            title: "This PDF doesn't match the official certificate",
            message:
              "A certificate with this ID exists, but the file you uploaded is different from the official version.",
            nextStep:
              "Treat this document with caution and request the original from the institution.",
          }
    : null;
  ...
  const resultStatusCopy = result
    ? result.status === "verified"
      ? {
          ...
          message:
            "This credential was found in our records and is currently active.",
          nextStep:
            "Review the details below to confirm the recipient and institution.",
        }
      : result.status === "pending"
        ? {
            title: "Certificate is pending",
            message:
              "This credential was found but hasn't been fully confirmed yet.",
            nextStep: "Check back later or contact the issuing institution.",
          }
        : {
            title: "Certificate is no longer valid",
            message:
              "This credential exists in our records but has been revoked.",
            nextStep:
              "Do not accept this certificate — contact the institution to understand why.",
          }
    : null;
  ...
  const docVerdictColors = documentVerdict
    ? verdictColors(documentVerdict.tone)
    : null;
  const docErrorColors = documentVerifyError
    ? verdictColors(
        documentVerifyError.kind === "ipfs_unreachable" ||
          documentVerifyError.kind === "no_ipfs_hash"
          ? "warning"
          : "danger",
      )
    : null;
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="mt-3 text-base text-gray-500 dark:text-gray-400">
            Check whether a certificate is real and was officially issued by an
        ...
        <div className="overflow-hidden rounded-none border-2 border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
          ...
          {activeTab === "pdf" && (
            <form onSubmit={handleDocumentVerify} className="p-6">
              <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">
                Upload the certificate PDF you received. We will compare it
                against the official copy to confirm it is genuine.
              </p>
          ...
          {activeTab === "id" && (
            <form onSubmit={handleSearch} className="p-6">
              <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">
                Enter the Certificate ID from your certificate document or the
                QR code you received.
              </p>
        ...
        {(isSearching || isVerifyingDocument) && (
          <div className="mt-8 flex flex-col items-center gap-3 py-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {isVerifyingDocument
                ? "Comparing your file to the official record…"
        ...
        {documentVerifyError && docErrorColors && !isVerifyingDocument && (
          <div
            className={`mt-6 rounded-none border-2 p-5 ${docErrorColors.container}`}
          >
            <div className="flex items-start gap-3">
              <docErrorColors.icon
                className={`mt-0.5 h-5 w-5 shrink-0 ${docErrorColors.iconClass}`}
              />
              <div className="min-w-0">
                <h4 className={`font-bold ${docErrorColors.title}`}>
                  {documentVerifyError.title}
                </h4>
                <p className="mt-1 text-sm text-gray-700 dark:text-gray-300">
                  {documentVerifyError.message}
                </p>
        ...
        {documentVerifyResult &&
          ...
          !isVerifyingDocument && (
            <div
              className={`mt-6 rounded-none border-2 p-5 ${docVerdictColors.container}`}
            >
              <div className="flex items-start gap-3">
                <docVerdictColors.icon
                  className={`mt-0.5 h-5 w-5 shrink-0 ${docVerdictColors.iconClass}`}
                />
                <div className="min-w-0 flex-1">
                  <h4 className={`font-bold ${docVerdictColors.title}`}>
                    {documentVerdict.title}
                  </h4>
                  <p className="mt-1 text-sm text-gray-700 dark:text-gray-300">
                    {documentVerdict.message}
                  </p>
                  <p className="mt-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                  ...
                  <div className="mt-4 grid gap-4 rounded-none border border-gray-200 bg-white/70 p-4 dark:border-gray-700 dark:bg-gray-900/40 sm:grid-cols-[1fr_auto]">
                    <div className="space-y-1.5 text-sm">
                      ...
                      <p className="text-gray-500 dark:text-gray-400">
                        ...
                        <span className="font-medium text-gray-700 dark:text-gray-300">
                          {documentVerifyResult.certificate.institution}
                        </span>
                      </p>
                      <p className="text-gray-500 dark:text-gray-400">
                        Issued on{" "}
                        <span className="font-medium text-gray-700 dark:text-gray-300">
                          {formatDate(
                            documentVerifyResult.certificate.issueDate,
                          )}
                        </span>
                      </p>
                    </div>
                    <div className="flex flex-col items-center gap-2">
                      <div className="flex h-20 w-20 items-center justify-center rounded-sm border border-gray-200 bg-white p-1 dark:border-gray-600">
                        <QRCodeSVG
                          value={getVerifyPageUrl(
                            documentVerifyResult.certId,
                            documentVerifyResult.verifyUrl,
                          )}
                          size={68}
                          level="M"
                          bgColor="#ffffff"
                  ...
                  <details className="mt-3 rounded-none border border-gray-200 bg-white/70 p-3 dark:border-gray-700 dark:bg-gray-900/40">
                    ...
                    <div className="mt-2 space-y-1.5 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <HashDisplay
                        ...
                        label="Your file (SHA-256)"
                        truncate={false}
                        className="text-[11px]"
                      />
                      <HashDisplay
                        hash={documentVerifyResult.ipfs.sha256}
                        label="Official file (SHA-256)"
                        truncate={false}
                        className="text-[11px]"
                      />
                    </div>
        ...
        {notFound && !isSearching && (
          <div className="mt-6 flex flex-col items-center gap-3 rounded-none border-2 border-red-200 bg-red-50 p-8 text-center dark:border-red-800 dark:bg-red-950/20">
            ...
            <p className="max-w-sm text-sm text-red-600/80 dark:text-red-400/70">
              {errorMsg ||
                `We couldn't find a certificate matching "${searchQuery}". Please double-check the ID or ask the issuer to share the QR code or verification link.`}
            </p>
          </div>
        )}
        ...
        {result && !isSearching && (
          <div className="mt-6">
            {/* Status banner */}
            <div
              className={`flex items-start gap-3 rounded-none border-2 p-4 ${
                result.status === "verified"
                  ? "border-green-300 bg-green-50 dark:border-green-800 dark:bg-green-950/20"
                  : result.status === "pending"
                    ? "border-yellow-300 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-950/20"
            ...
            >
              {result.status === "verified" ? (
                ...
                <Clock className="mt-0.5 h-6 w-6 shrink-0 text-yellow-500" />
              ) : (
                <XCircle className="mt-0.5 h-6 w-6 shrink-0 text-red-500" />
              )}
              <div>
                <h3
                  className={`text-lg font-bold ${result.status === "verified" ? "text-green-700 dark:text-green-400" : result.status === "pending" ? "text-yellow-700 dark:text-yellow-400" : "text-red-700 dark:text-red-400"}`}
                >
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-[1fr_auto]">
                ...
                <div className="divide-y divide-gray-100 dark:divide-gray-800">
                  {[
                    ...
                    >
                      ...
                      <span
                        className={`text-right text-sm text-gray-900 dark:text-white ${(row as { mono?: boolean }).mono ? "font-mono" : ""}`}
                      >
                        {row.value}
                      </span>
                    </div>
              ...
              <div className="flex flex-col items-center gap-3 rounded-none border-2 border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
                ...
                <p className="max-w-[10rem] text-center text-xs text-gray-500 dark:text-gray-400">
                  Scan to share this verification
                </p>
                <a
